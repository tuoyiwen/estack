...
| 字段 | 内容 |
|---|---|
| Title | 编程是通用性的特例，验证成本决定 token 经济能否复制 |
| Type | research anchor / boundary condition |
| Source | [[../archive/original-sources/2026-07-05_腾讯研究院_Token不经济_原文\|Token 不经济归档]] |
| Relevant skill | `$e-innovation-map`, `$e-innovation-metrics`, `$e-ai-governance` |
| What it helps decide | 判断一个场景的 AI 收益能否复制编程场景的成功，核心看反馈信号可得性和验证成本。 |
| Reusable output | use case 选型维度（反馈信号可得性）；桥梁场景清单；对"AI 在编程上行所以处处行"的反驳 |
| Confidence | direct（机制论证）+ partial（具体案例数字） |

### 机制含义

编程场景有两个别处几乎不存在的条件：确定的信号反馈（编译器、测试立刻给出无歧义对错）和自动的后训练闭环（反馈直接接入强化学习回路）。离开编程，验证成本会吃掉 token 经济：律师复核 AI 标注合同的时间几乎等于从头审阅（Harvey AI / A&O 案例）；物理世界不接受迭代只接受验证，验证成本永远高于生成成本（Sim-to-Real gap）。

务实的中间地带是弱确定性反馈场景：影像筛查（有影像学标准）、需求预测（有历史数据回测）、合同初筛（有条款模板比对）。验证成本不趋近于零，但远低于物理世界。

### 和论文方向的连接

这条与快手案例的"验证规模化"难题、四类协作摩擦里的"验证与纠偏速度不对称"是同一机制的三个观测点：人机协作的净收益由验证/评价环节的成本结构决定。对博论工序链框架，它支持把"验证与选择环节的反馈信号质量"作为人机配置效果的边界条件。

## Taste 与策展：实现廉价化之后的末端选择瓶颈

| 字段 | 内容 |
|---|---|
| Title | 实现变廉价后，人类判断上移到策展与选择 |
| Type | research anchor / human judgment mechanism |
| Source | [[../archive/original-sources/2026-07-05_Lennys-Podcast_Codex-Andrew-Ambrosino_transcript\|Codex 访谈归档]] |
| Relevant skill | `$e-collaboration-design`, `$e-innovation-workflow` |
| What it helps decide | 设计人机分工时，把人的稀缺判断放在哪个环节。 |
| Reusable output | 流程倒置叙事；taste 的四个构成维度；角色流动性论据 |
| Confidence | direct（OpenAI Codex 团队一手实践） |

### 机制含义

当实现成本趋近于零（同一功能公司内可有 90 个并行原型），贵的环节从生产转移到策展：评估平行尝试、判断什么值得留下、如何折叠进整体。Taste 不只是审美，包含系统思维（如何嵌入现有系统）、情境意识（为什么该存在）、语义执行、媒介选择四个维度。配套的组织变化是角色流动化："角色 = 时间分配的平均"，但专业最佳实践不能随角色边界一起被取消。

### 和论文方向的连接

这是工序链框架“末端选择”环节目前最鲜活的行业例证：AI 把生成产能推向过剩后，novelty/usefulness 的选择判断成为人类价值集中点。“90 个原型 + 策展”可直接用作研一（末端选择）的现实动机案例；“验证与纠偏速度不对称”（Token 不经济）提供同一机制的成本侧证据。

## AI 身份分化：同一技术的分化效应（sorting effect）

| 字段 | 内容 |
|---|---|
| Title | AI 对专业身份的分化效应：amplified vs destabilized |
| Type | research anchor / individual-level mechanism |
| Source | [[../archive/original-sources/2026-07-08_Lennys-Newsletter_Tech-Worker-Sentiment-2026_原文\|Lenny 2026 情绪调查归档]] |
| Relevant skill | `$e-ai-governance`, `$e-collaboration-design`, `$e-innovation-pilot` |
| What it helps decide | 解释为什么同一项 adoption 在同一组织内产生完全相反的员工结果，以及 pilot 为什么要按身份立场分层设计支持。 |
| Reusable output | 身份四分布基线；四类型画像（Energized/Conflicted/Disoriented/Resentful）；sorting risk 治理立场 |
| Confidence | direct（对 product-centric 从业者，N=5,332）；外推到全体 tech workers 为 partial（自选样本、practitioner 级测量） |

### 机制含义

“AI 如何改变你作为专业人士的自我认知”这一个问题（amplified 49% / redefined 27% / destabilized 14% / diminished 5%）对职业乐观度的预测力（β=+0.39；两端差 d≈1.55）超过角色、级别、公司规模之和。也就是说：**决定员工 AI 时代感受的不是他做什么工作，而是 AI 对他自我价值感做了什么。** 情绪聚类的四类型（Energized 41% / Conflicted 35% / Disoriented 12% / Resentful 12%）显示大多数人处在矛盾中间层（77% 同时持有正负情绪，smiling exhaustion）。

配套发现：主导恐惧是 squeeze 不是替代（51% 怕同酬更多产出 vs 22% 怕被取代；“AI 接管我的工作”与裁员焦虑相关 r≈0）。

### 和论文方向的连接

- amplified/destabilized 是现成的 professional identity 测量思路，与 HAI 文献中 identity threat / professional identity work 方向相通；博论若做个体层调节，这是一个可操作化的构念雏形（注意它是 practitioner 测量，需换 validated 量表）。
- 与 [[research-anchors#Organization absorption layer|organization absorption layer]] 互补：吸收层讲组织谁来消化 AI 产出，身份分化讲个体谁被抬升谁被动摇；两者共同解释"同样的 AI 能力，不同的组织与个体结果"。
- “质量与认知敏锐度下滑”的开放题证据（"my brain is rotting"、"I just follow Claude"）是 cognitive surrender（Osmani）与 [[../archive/original-sources/2026-07-05_腾讯研究院_Token不经济_原文|Token 不经济]] 认知投降风险的员工侧实证回声，也贴近 Berfu 案例的 non-reflective execution。
